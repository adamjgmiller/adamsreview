...
- **(a) Stop — I'll merge `$merge_ref` into `$head_branch` first, then re-run.** Exit 0 with: `Stopping. Run \`git merge $merge_ref\` (or fast-forward) on \`$head_branch\`, then re-run /adamsreview:add.`
