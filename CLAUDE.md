...
| `branch-behind-base.sh` | Bash | Preflight check for the rotated staleness axis (HEAD vs `$comparison_ref`) — companion to `freshness-gate.sh` (which covers local_base vs remote_base). Two mutually-exclusive modes: passive (`--comparison-ref <ref>`, no fetch — used by `:review` 0.6a; 0.2a already fetched origin/<base> if the freshness path needed it, or determined the repo was no_remote / fetch_failed and fell back to local <base>, so 0.6a doesn't re-attempt), and active-fetch (`--fetch-base <name>`, runs `git fetch origin <base>` with the same 30s soft timeout pattern as `freshness-gate.sh`, falls back to local `<base>` on no_remote / fetch failure — used by `:fix` 7.6a and `:add` 3a so the freshest signal is checked at fix/add time). Reads the staleness envelope as `--reviewed-files @-` (stdin) or inline; both forms must be non-empty (a real review's `reviewed_files_all` is non-empty by construction — empty input is a caller bug, exits 64). Emits JSON `{behind_count, overlap_count, overlap_files[], comparison_ref_used, warnings[]}` with `overlap_files` truncated to the first 10 paths plus a `"…+N more"` sentinel. Behind = 0 short-circuits without computing the diff. Orphan / unrelated histories (no merge-base) collapse to a single coherent state: `behind_count=0`, `overlap_count=0`, plus a `no_merge_base` warning — orphan has no "behind-base" relationship to surface, so the caller's `behind == 0` short-circuit naturally skips the AskUserQuestion. Active-fetch call sites detect a fetch-fallback (`comparison_ref_used == base_branch` rather than `origin/base_branch`) and append "(fetch failed or no remote — comparing against local)" to the prompt body so the user can tell a fallback comparison from a successful fetch. All three integration sites fail-open if the helper itself exits non-zero (logged as `helper_failed rc=N` to trace.md and behind treated as 0); the gate is a warning surface, not a precondition. The orchestrator surfaces the result via `AskUserQuestion` (Stop / Proceed / Abort) when behind > 0, with the prompt body adapting to whether the file-overlap is empty (latent-baseline risk) or non-empty (phantom-deletion risk + listed paths). |
