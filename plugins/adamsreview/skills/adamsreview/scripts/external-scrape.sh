...
#   .codex/review-config.json                       (per-repo, cwd-relative)
#   .claude/review-config.json                      (legacy fallback)
...
resolve_config_path() {
    ...
    if [[ -f ".codex/review-config.json" ]]; then
        echo ".codex/review-config.json"
        return
    fi
...
if [[ -n "$FIXTURES_DIR" ]]; then
    ...
    if [[ "$FAIL" -ne 0 ]]; then
        ...
        if grep -qi "rate limit" "$ISSUE_ERR" "$REVIEW_ERR" "$RVCMT_ERR" 2>/dev/null; then
            echo "Action: wait for the rate limit to reset (see 'X-RateLimit-Reset' headers above), or re-run \$adamsreview review; PR-comment enrichment is fail-open." >&2
        else
