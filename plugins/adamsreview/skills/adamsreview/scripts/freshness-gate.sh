...
#
# The `Codex user prompt` dispatch stays orchestrator-side: this helper
...
\`behind_count\`, and \`preflight_warnings\`.
  ...
  --after-choice   Optional. When re-invoked after Codex user prompt:
...
Codex user prompt flow and the prompt text shown to the user.
USAGE
}
...
# Codex user prompt. Set false after a non-FF (a)-retry failure.
emit_pending() {
