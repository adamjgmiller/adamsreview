...
def render_header(artifact):
    ...
    # (see project instruction files §"Pipeline shape" for the rationale).
    if turn_count:
        lines.append(
            "**Orchestrator tokens:** "
...
def render_summary(buckets):
    ...
    # fix mode can land in any of these states, and would otherwise drop from the
...
def render_deep_other(buckets, disposition):
    ...
    if disposition == "confirmed_manual":
        lines.append(
            ...
            "Findings with an auto-recommendation get batch-confirmed during fix "
...
def render_light_lane(buckets):
    ...
    # and then fixed by fix mode can land in any of these states, and would
    ...
    if any(f.get("disposition") in ("confirmed_mechanical", "confirmed_manual") for f in rows):
        lines.append(
            ...
            "batch-confirmed during fix preflight (or walkthrough Step 4.5); "
...
def render_footer(artifact):
    return "Generated with the [adamsreview](https://github.com/adamjgmiller/adamsreview) Codex Review Plugin"
